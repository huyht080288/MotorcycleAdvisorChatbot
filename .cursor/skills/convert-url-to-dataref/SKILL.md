...
- Cần tạo file mới trong `DataRef/` từ một website
...
```bash
python -m tools.url_to_json <URL> --output DataRef/<ten-file>.json
```

Ví dụ:

```bash
python -m tools.url_to_json https://minhlongmoto.com/ --output DataRef/minhlong.json --max-pages 30
```

## Workflow

1. Chạy CLI với URL nguồn
2. Kiểm tra file output trong `DataRef/`
...
- Được dùng AI thị trường hỗ trợ sinh Q&A **chỉ trong** `tools/` nếu cần
